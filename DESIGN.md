...
**수집 후속 이력 — 2026-09-15:** 상세 수집 job의 성공과 reader/이미지/조문 처리 종료를 구분한다. 최근 출처 50건의 서버 이력을 새로고침 후에도 조회하고 reader 미등록 건수·진행 중·확인 필요·단계별 실패/미연결을 표시한다. 후속 처리 종료를 모든 자료 취득 성공으로 표현하지 않는다.

**관리자 백업 — 2026-09-14:** 백업 관리에서 명시적 실행·최근 이력·파일 진행 수와 실패 상태를 제공한다. 불확실한 제출은 같은 요청 ID로 확인하고 새로고침 후 DB 이력을 복구한다. Parquet 포함 여부, 외부 archive 미포함 및 복원 미검증을 구분하며 백업 성공을 복원 성공으로 표시하지 않는다.

**이미지 재취득·검색 색인 — 2026-09-14:** 현재 수집 본문의 이미지 재취득은 위치와 제공자 주소를 유지하고 후속 reader 발급까지 상태를 표시한다. 검색 색인 관리는 관리자 영역에서 펼쳐 실행 범위·처리 행 수를 확인한다. 두 실행 모두 요청 식별자를 유지하고 새로고침 후 진행 상태를 복구한다.

**현재 본문 조문 재확인 — 2026-09-14:** 관리자만 현재 수집 본문에서 제공자 조문 재확인을 실행한다. 요청 ID를 유지한 재확인, 새로고침 후 상태 복구, 완료 결과로의 명시적 이동을 제공한다. 완료 job은 모든 조문 취득 또는 적용 법령 버전 확인을 뜻하지 않으므로 위치별 상태를 계속 표시한다.

**검색 판례의 보강 본문 — 2026-09-11 사용자 확정:** 검색 결과를 클릭하면 scourt HTML의 문서 구조·문단·표·순서를 유지한 본문에 scourt 이미지와 lawgo 법령·조문 보강 내용을 함께 표시하는 것을 목표로 한다. 이미지는 원래 등장 위치마다 내부 보존 파일로 표시하고, 조문은 원래 인용 위치와 연결하여 같은 열람 화면에서 내용을 확인할 수 있어야 한다. 외부 사이트로 이동해야만 조문을 볼 수 있는 링크만으로 완료 처리하지 않는다. 원본 HTML은 보존하고 안전한 파생 화면에서 필요한 부분을 교체·보강한다. 미취득 이미지, 조문 미연결/실패, 법령 버전 미확인은 해당 위치에서 구분한다. 이 기준은 기존 89,130행 전체와 앞으로 수집할 신규 판례 모두에 필수다. KorCounsel 프런트의 일반 검색 결과에서 동일한 보강 열람 경험을 제공해야 하며 별도 검증 표본 화면에만 한정하지 않는다. 현재 구현 완료 보고가 아니다. [보강 계약](docs/legacy-enrichment-and-incremental.md#검색에서-완전한-보강-본문-열람까지).

**로그인 랜딩 — 2026-09-11:** 미인증 첫 화면은 아이디·비밀번호 플레이스홀더를 가진 중앙 로그인 폼이다. 검색·본문을 노출하지 않으며 로그인 후 서버가 확인한 관리자/편집자 역할을 표시한다. 역할 선택·공개 회원가입은 없다. [서버 계약](docs/site-login.md).

**이미지 열람 구현 현황 — 2026-09-11:** 로그인·검색·본문 열기 및 별도 현재 본문 표본의 저장 이미지 표시를 구현했다. 원문 해시별 위치 manifest, 실패 위치 안내, 세션 만료 시 화면 비우기, 직접 링크·새로고침 복원을 적용한다. [실제 검증 범위](docs/image-reader-validation.md).
