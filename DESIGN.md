...
**사건기호 상세 표시 — 2026-09-15:** 60필드의 `case_sort`는 원 사건기호와 함께 분야·심급·재판부·절차를 조합해 표시한다. 예: `가단 · 민사 · 제1심 · 단독`. 공식 설명과 시간 적용 상태는 근거 상세에서 확인하며, 과거 시행본 미확인은 검증 보류 문구로 드러낸다. 구조화되지 않은 기존 문자열도 그대로 읽을 수 있어야 한다. [분류 계약](docs/case-symbol-classification.md).

**품질 점검·선택 재처리 — 2026-09-15:** 관리자 화면은 저장된 현재 reader의 품질을 일시 실패·로직 점검·사람 검토·정보 미제공·미처리/연결 대기로 나눠 판례 수와 위치 수를 표시한다. 기간 또는 전체 점검, 유형 필터, 25건 페이지, 최대 20건 선택, 60필드 생성과 일시적 이미지/조문 실패 재처리를 제공한다. 구조 변경이나 동일성 충돌을 재시도 버튼으로 숨기지 않는다. 점검·재처리 job ID와 결과는 새로고침 뒤 복구하며, 상위 job 종료와 후속 chain 종료를 구분한다.

**기간별 증보 — 2026-09-15:** 관리자 증보 영역에서 선고일 시작/종료를 함께 입력하고 최대 93일 범위를 지정한다. 페이지 수·페이지당 건수·관찰 범위를 표시한다. 전체 기간 목록 확인 뒤 불변 후보를 오래된 선고일부터 최대 50건씩 등록하며 등록 위치·후속 묶음·후보 소진 상태를 표시한다. 같은 관찰의 후보 재계산/재요청은 기존 상세 job을 재사용한다. inventory job ID는 URL에 유지해 새로고침 후 상태를 복구한다. 목록 관찰, 본문 단계 종료, 60필드/이미지/조문 처리 및 검증 보류를 구분한다. 일부 목록만으로 기간 전체 등록 완료를 표시하지 않는다. [실행 기록](docs/chronological-ingestion-window.md).

**60필드 조회 구현 — 2026-09-15:** 일반 검색 상세에서 보강 본문/60필드를 전환한다. 60개 값을 숨기지 않고 필드별 상태·근거·규칙·revision을 펼쳐 보며 URL view=fields로 새로고침을 복원한다. 신규 REVIEW/미처리와 기존 LEGACY_STORED를 구분한다. 관리자 필드 생성/같은 요청 확인 버튼과 영속 처리 이력에 연결했다. 값 직접 편집·승인은 이번 구현에 포함하지 않는다. [상세](docs/case-fields-contract.md).


## 60필드 조회 — 2026-09-15 사용자 확정

기존 89,130행과 신규 판례에 동일한 60필드 조회 화면을 제공한다. 일반 검색→판례 상세에서 ‘보강 본문 / 60필드’를 전환하며, 별도 관리자 점검 화면에만 한정하지 않는다. 인증된 사용자가 서버 권한 확인을 거쳐 조회한다. 현재 조회 화면은 구현됐으며 아래 계약을 유지한다.

- 원래 60개 필드 모두를 한글 설명·원래 필드명·값으로 확인한다. 빈 필드도 누락하지 않는다. 긴 본문·HTML 문자열과 목록/사전 등 복합값은 펼쳐 볼 수 있게 하며 타입이나 값의 차이를 임의로 합치지 않는다. 원문 HTML은 실행되지 않는 안전한 표현으로 제공한다.
- 값 존재와 처리/검증 상태는 별개로 표시한다. 정보 미제공·비해당·미처리·추출 실패·검증 보류를 구분하며, 과거 sentinel의 의미가 확정되지 않았으면 미확인으로 남긴다.
- 추출 근거가 확인된 필드는 해당 원문/metadata와 대조할 수 있게 한다. 검증한 위치가 있을 때만 강조하고, 위치 미확인을 임의 연결로 메우지 않는다.
- 생성 규칙 버전·처리 시각·원문 참조·파생 revision을 이력 상세로 제공한다. 보존되지 않은 과거 이력은 미확인으로 표시한다.
- 선택한 필드 결과와 reader revision의 대응 관계를 명시한다. 새로고침과 직접 링크에서 판례/선택 상태를 복원하고, 다른 판례 선택·로그아웃 이후 늦은 응답이 화면을 덮지 않도록 한다.
- 기존/신규 대표 판례 및 신규 23건에서 60필드 누락 없는 조회, 저장값·타입 대조, 긴 값/복합값·결측/오류 상태·원문 왕복 이동·인증 만료를 검증한다.

필드 조회는 수집 정리 완료 확인에 포함한다. 필드 직접 수정·승인 기능은 이 요구에 포함하지 않고 후속 검수 단계에서 다룬다.
