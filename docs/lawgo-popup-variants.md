...
## 최근 선고 표본 확대 — 2026-09-14

보유 legacy 검색 projection에서 선고일 2024-01-01 이후이며 별표를 언급하는 행을 선고일 내림차순·ordinal 순으로 30개 조회하고, 서로 다른 lawgo ID 5개를 선정했다. 선정 목록은 `recent-annex-audit-20260914:selection`에 먼저 보존했다. 보유 corpus 기준 최근 표본이며 공식 전체 최신 inventory를 조회한 것이 아니다. 2025~2026년 판례의 별표 제공 여부는 이 조사로 판단할 수 없다.

| 대법원 판례 | 선고일 | lawgo ID | 별표 언급 | 지원 JO 링크 | 별표 문구 링크 |
| --- | --- | --- | ---: | ---: | ---: |
| 2024도8707 | 2024-09-27 | 241637 | 3 | 22 | 0 |
| 2023도1924 | 2024-08-23 | 241497 | 4 | 30 | 0 |
| 2022두60073 | 2024-08-01 | 241375 | 18 | 20 | 0 |
| 2024다230251 | 2024-07-25 | 241223 | 1 | 36 | 0 |
| 2022두48837 | 2024-07-25 | 241237 | 4 | 11 | 0 |

현재 공식 precInfoP 응답을 각 1회 요청해 보존했다. frame 내부 precSeq 일치, 선고일과 저장 metadata 날짜 일치, 보존 후 SHA-256 재대조가 모두 통과했다. script/style/comment를 제외한 frame 텍스트의 별표 언급은 총 30곳이며, 본문만의 정밀 occurrence 수는 아니다. a 태그의 별표·별지·서식 문구 링크와 inline handler의 비JO fncLawPop은 모두 0개다. 지원 JO 링크 119개는 별표의 실제 연결을 의미하지 않는다. 브라우저의 동적 DOM이나 JO 팝업에서 파생되는 모든 추가 탐색 경로를 전수 조사한 것은 아니다.

원본 ID는 `recent-annex-audit-20260914:frame:<lawgo ID>`이며 최종 감사 artifact는 `recent-annex-audit:d9ec4ebf6810bfaeb6ed797e5487abeebb4dcddb62090295e179bb135ff8d839`이다. 기존 source/reader manifest를 변경하지 않았고 별표/조문 추가 취득 job을 등록하지 않았다. 실제 제공 링크가 관찰되지 않아 새 형식 지원 구현은 보류한다. 반복적인 동일 형식 표본 조사만으로 전체 미제공을 단정하지 않는다. 이후 실제 비JO 링크가 발견되면 그 원본·위치·호출 형식을 기준으로 구현 범위를 다시 연다.

이번 변경은 실제 HTTP 표본 조사와 문서 기록이다. 앱 코드·UI 변경이 없어 전체 Python/프런트 회귀와 브라우저 검색 검증을 재실행하지 않았으며 문서 `git diff --check`를 수행했다.
