...
**후속 사용자 결정:** 이 문서의 146행 검증은 교정 전 원행 보존 실험이다. 실제 전수 Parquet은 기준 원문을 유지하면서 확인된 추출 오류를 교정한 결과로 만든다. 역사적 OPAQUE 보존을 운영 필드의 오류 유지 의무로 해석하지 않는다. [원문 보존과 교정 계약](legacy-full-row-import.md#원문-보존과-추출값-교정--2026-09-10-사용자-확정).
