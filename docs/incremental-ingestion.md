...
**2026-09-15 제한 품질 교정:** 고정 433개 source의 현재 실집계와 법원명 공백
충돌 4개·HTTP 200 서비스 오류 1곳의 제한 재처리를 수행했다. `current-lawgo-3`는
법원명 공백만 비교에서 제외하며 지원·병합 번호·재판 종류·날짜 검증은 유지한다.
알려진 `div#error500`과 서비스 오류 문구가 함께 있는 응답은
`LAWGO_SERVICE_UNAVAILABLE`로 분류해 제한 재시도 가능 대상으로 추가했다.
팝업 이동만 있는 응답은 구조 실패로 남긴다. 실제 실행 결과와 미해결 근거는
[최신 기준선](quality-baseline-20260915.md)을 따른다.

## 관리자 품질 점검과 선택 재처리 — 2026-09-15

관리자 명시 명령으로 저장된 CURRENT_SOURCE의 source별 최신 reader를 품질 점검한다. 날짜 범위는 양쪽 날짜와 최대 93일 계약을 사용하고, 날짜를 비우면 현재 reader 전체를 고정한다. 기존 89,130행 Parquet와 reader 미등록 자료는 이 점검 범위가 아니며 화면에 범위를 명시한다.

점검은 외부 제공자에 접속하지 않고 보존 reader·60필드·본문을 읽는다. 명시 표제가 있는데 필드가 비거나 주문에 이유가 섞인 조용한 누락, 필드 ERROR/REVIEW/NOT_PROCESSED/NOT_PROVIDED, 이미지 위치 상태, 조문 PRESERVED/FAILED/AMBIGUOUS/UNLINKED, lawgo 동일성과 법령 버전 미확인을 판례 수와 위치 수로 집계한다. 결과와 판례별 위치는 immutable artifact로 저장하며 중단 후 완료 항목을 재사용한다.

재처리는 점검 보고서에서 최대 20건을 명시 선택한다. 60필드 생성은 현재 규칙의 immutable 결과를 재사용하거나 새 revision을 만든다. 이미지와 제공 조문은 network failure, HTTP 408/429/500/502/503/504 및 확인된 LAWGO_SERVICE_UNAVAILABLE 응답을 제한 재시도한다. 구조 변경, 빈/손상 응답, 401/403/404, 미연결, 동일성 충돌은 일시 실패 재시도 대상이 아니다. 실행 직전에 source별 최신 reader와 같은 source의 진행 중 작업을 다시 확인하며, 중단 직전 등록한 자식 job도 request key로 복구한다. 이미지·조문 재처리 뒤 새 reader와 그 reader의 60필드까지 영속 chain으로 추적한다.

UI는 전체/유형 필터, 25건 pagination, 문제 위치·원인 코드, 본문·60필드 이동, 점검/재처리 이력을 제공한다. 브라우저를 닫아도 worker가 계속하며 불확실한 제출은 같은 요청으로 확인한다. 작업 종료를 품질 해결로 표시하지 않고 재점검하도록 안내한다. 정기 schedule이나 AI 판단은 사용하지 않는다. 현재 규칙은 알려진 오류 탐지의 첫 revision이며, 새 오류 표본은 별도 규칙 version과 회귀 fixture로 확장한다.

실제 2024-09~12 고정 대상 410건을 두 품질 job(`846501c3-907f-4eec-8ee1-56ab59d19806`, `154ad54a-1aaa-4fa1-a3e7-83c5959dad1d`)으로 점검했다. LOGIC_REQUIRED는 5건·20곳이며 모두 이미 보존된 `LAWGO_ARTICLE_STRUCTURE_CHANGED`이고, 새 60필드 조용한 누락은 0건이다. RETRYABLE은 0건이라 자동 외부 재요청을 만들지 않았다. PENDING은 157건·2,142곳, REVIEW는 410건·6,455곳, 정상 정보 미제공은 410건·7,690필드다. 판례 1건의 실제 60필드 선택 재처리 job `45ea0c1b-c538-4efe-8ce5-602f8743398c`은 부모·필드 job 모두 성공했고 기존 reader bytes는 불변이었다. Chromium에서 저장된 점검 이력 복구, 유형별 1건/1위치 필터, 20건 선택 제한 안내와 본문·60필드 링크를 확인했다.
