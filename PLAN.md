...
기존 충돌 35개 재처리·60필드 재발행 후 고정 433개에서 EXACT 364 /
CONFLICT 0 / UNMATCHED 69, 조문 보존 6,554곳이다. 조문 실패 23곳과
폐기·법령 버전 보류는 별개다. [원인·구현·검증·snapshot](docs/lawgo-full-docket-correction.md).
...
공백 충돌 4개를 `current-lawgo-3`로 교정해 lawgo EXACT 329 / CONFLICT 35 /
UNMATCHED 69, 조문 보존 5,927곳을 확인했다. 실패 21곳은 팝업 이동만 제공된
20곳과 재시도 후에도 남은 서비스 오류 1곳으로 분리했다. 새 reader·v6 필드
5개와 로컬 433행 snapshot을 추가했고 과거 결과는 보존했다. 전체 회귀
791 passed / 선택 백업 시험 3 skipped. [실측·SQL·교정·보류·artifact](docs/quality-baseline-20260915.md).
다음 기간 수집·역사적 기호 전수 재발행·AWS 이관은 이번 실행에 포함하지 않았다.

**새 작업 인수인계 — 2026-09-15:** 현재 고정 신규 cohort는 시험 23건과
2024-09~12 연속 구간 410건을 합친 433건이며, 사건기호 case-fields-6
발행까지 완료됐다. 다음 작업의 정확한 순서, 재집계 기준, 전체 로컬 수집 후
AWS snapshot 이관 계획은 [새 Codex 작업 인수인계](docs/HANDOFF-20260915.md)를
우선 확인한다. 아래의 과거 신규 23건 미완료 체크리스트는 당시 기록이다.

**사건기호 구조화 — 2026-09-15:** case-fields-6에서 원 기호를 보존하고 공식 코드·분야·심급·재판부·절차·공식 설명을 구조화한다. 신규 고정 snapshot 433건의 27개 기호를 지원하고 v6를 재발행했으며 기존 REVIEW 184건을 포함해 case_sort PRESENT 433건을 확인했다. 2022-07-29 이전 판례는 당시 예규 시행본을 확인하기 전까지 REVIEW다. 기존 89,130행과 v5 이하 revision은 불변이다. [설계와 근거](docs/case-symbol-classification.md).

**관리자 품질 점검·선택 재처리 — 2026-09-15:** 저장된 CURRENT_SOURCE의 source별 최신 reader를 날짜 범위 또는 전체 범위로 고정해, 60필드 상태·명시 표제 대비 누락·주문/이유 혼입·이미지·제공 조문·lawgo 연결·법령 버전 미확인을 결정론적으로 집계하는 영속 품질 job을 구현했다. 결과는 불변 보고서이며 일시 실패, 로직/입력 점검, 사람 검토, 정보 미제공, 미처리/연결 대기를 구분한다. 관리자는 최대 20건을 골라 60필드 생성 또는 허용 목록의 일시적 이미지/조문 실패만 재처리할 수 있다. 구조 변경·404·인증·decode·동일성 충돌은 무한 재시도하지 않는다. 재처리 시 최신 reader와 진행 중 job을 다시 확인하고, 새 reader 및 새 60필드 revision을 기존 결과와 함께 보존한다. 정기 실행은 없다. [상세 계약](docs/incremental-ingestion.md#관리자-품질-점검과-선택-재처리--2026-09-15).

실제 2024-09~12의 410건 품질 점검에서 새 조용한 필드 누락은 없었고, 로직 점검 20곳은 모두 기존 LAWGO 조문 구조 변경 실패였다. 자동 재시도 가능한 일시 실패는 0건이었다.


**오래된 구간부터 3개월 증보 — 2026-09-15:** 기존 89,130행의 마지막 선고일은 2024-09-27이다. 현재 9월 목록 87건 중 기존 대응 30건·미대응 후보 57건이며, 10~12월 미대응 후보는 353건이다. 첫 작업은 9월 경계 보완 57건 → 10~12월 353건 순서로 잡는다. 날짜 범위·기간 목록 완전성 검사·선고일 오름차순 묶음 등록·같은 관찰 재요청 중복 방지를 관리자 경로에 구현했다. 사용자 승인으로 9월 57건·10월 116건·11월 111건·12월 126건, 총 410건의 원문/reader/후속 처리/60필드 생성을 실행했다. 실제 발견한 각주 제목 오류는 case-fields-4, 공백이 든 당사자 제목 오류는 case-fields-5로 교정하고 이전 결과는 보존했다. 앞선 시험 23건과 합친 433행×60열 snapshot을 발행했으며 의미 검증 보류와 제공 조문 실패는 별도 상태로 유지한다. 이후 2025년부터 순차적으로 2026-08-31까지 확대한다. [근거·한계](docs/chronological-ingestion-window.md).
...
**로컬 완료 후 AWS 복사 이관 — 2026-09-15 사용자 확정:** 전체 수집 및 기존/신규 60필드 정리·검증과 reader/보강 처리를 로컬에서 완료한 후, 그 시점의 일관된 DB·파일 snapshot을 AWS에 복사·복원한다. AWS에서 초기 corpus를 재수집하지 않는다. 이관 전 완료 범위와 미확정 상태를 고정하고 이관 후 무결성·서비스 동작을 대조한다. 로컬 원본은 유지하며 현재는 계획만 기록한다. 이관 이후 신규 증보의 실행 위치·운영 방식은 별도 결정 사항이다.

## 최초 우선순위와 완료 기준 — 2026-09-15 당시 계획 이력

아래 23건·UI·공통 추출 경로의 미완료 표현은 최초 계획이다. 현재 433개 v6
발행과 조회·품질 운영 구현은 상단 최신 기록을 따른다. 남은 실제 범위는
[현재 잔여 작업](docs/remaining-work-20260915.md)의 최신 요약에서 관리한다.

**신규 판례도 기존과 같은 60필드 생성·검증까지 마쳐야 수집·보존·정리 완료다.** 현재 신규 23건의 원문·reader/보강 처리 완료는 이 전체 완료를 뜻하지 않으며 60필드 생성·검증은 남아 있다. 기존 89,130행의 추출 결과를 재사용하고, 확인된 오류는 복제하지 않는다.

1. 60필드별 의미·원천·타입·결측·추출/검증 규칙을 대조해 공통 계약을 만든다.
2. 신규 23건에 적용하고 새 파생 artifact/Parquet·DB 조회·reader 연결을 전수 검증한다. 기존·신규 공통 60필드 조회 UI에서 값·상태·원문 근거·이력을 확인하는 것까지 포함한다.
3. 관리자 웹 수집에 60필드 단계를 포함하고 단계별 진행/실패/전체 완료·재시도를 구분한다. 증분 비교에 이미 수집한 신규 ID와 미완료 ledger도 반영한다.
4. 기존/신규 동일성·canonical 연결 및 필드/이미지/조문 미확정 품질을 정리한다.
5. 기존 필드를 바탕으로 쟁점·답변·원문 근거 구조화, 검수와 버전 있는 dataset 발행을 구현한다.
6. 별도 저장소 백업·웹 검증·실서비스 운영을 준비한다. 정기 schedule/cron은 만들지 않는다.

[상세 잔여 작업·검증 기준](docs/remaining-work-20260915.md). 아래 날짜별 보고와 과거 Step 체크박스는 당시 범위를 기록한 것이며, 이 최신 완료 기준과 충돌하는 부분은 그대로 실행하지 않는다.
...
**날짜 지정 조문 브라우저 검증 완료 — 2026-09-14:** WSL Chromium으로 2010두9976 일반 검색→최신 reader 1건→조문 24곳 보강·2곳 미연결 표시와 인용 왕복 이동을 검증했다. 새로고침 복원·로그아웃 화면 제거·미인증 검색/본문 401, 외부 요청 0건을 확인했다. 앱 코드·원본·manifest 변경 없음. [실증 기록](docs/lawgo-popup-variants.md#실제-브라우저-검증-완료--2026-09-14-wsl-후속).
