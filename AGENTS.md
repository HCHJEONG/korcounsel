...
고유 판례 총수로 합산하지 않는다. 법원명 공백 비교는 `current-lawgo-3`, 필드는
`case-fields-6`이다. 교정 후 lawgo EXACT 329 / CONFLICT 35 / UNMATCHED 69,
조문 보존 5,927 / 실패 21곳을 확인했다. 최신 시각·SQL·snapshot과 보류 근거는
[품질 기준선 기록](docs/quality-baseline-20260915.md)을 따른다. 아래 과거 v3~v5,
신규 23건 미완료 문구는 당시 이력이며 현재 할 일로 재실행하지 않는다.

**첫 연속 구간 증보 — 2026-09-15:** 2024-09 경계 보완 57건 → 10월 116건 → 11월 111건 → 12월 126건, 총 410건을 관리자 명시 요청으로 실행했다. 날짜 범위·목록 누락 검사·오래된 선고일 순 묶음·같은 관찰 재요청 중복 방지를 연결했다. 원문/reader/이미지·제공 조문 후속 처리와 60필드 생성을 종료하고, 실제 각주 제목 `【이유주1)】` 추출 오류는 새 `case-fields-4` revision으로 교정했다. 앞선 시험 23건과 합친 433행×60열 snapshot을 발행했다. 의미 검증 보류·조문 미연결/실패·법령 버전 미확인은 남으며, 상세 최종 검증과 artifact는 [기간 증보 기록](docs/chronological-ingestion-window.md)을 따른다. 과거 v3 snapshot은 이력으로 보존하며 당사자 제목 공백과 항소인 표제 인식도 `case-fields-5`로 교정했으며 현재 필드 규칙은 v5다. 기존 89,130행 Parquet·원문·기존 reader는 불변이다. DB CURRENT_SOURCE 전체 개수를 신규 판례 수로 해석하지 않는다.
...
**60필드 조회 UI — 2026-09-15 사용자 확정:** 기존·신규 판례 모두 일반 검색에서 판례 상세로 진입해 동일한 60필드 전체를 조회할 수 있어야 한다. 보강 본문과 60필드 보기를 전환하고 필드명·값·처리/검증 상태·확인 가능한 원문 근거·생성 이력을 제공한다. 결측 필드를 숨기거나 미처리/추출 실패를 정보 없음으로 표현하지 않는다. 보존되지 않은 과거 규칙·시각·원문 위치는 꾸며내지 않고 미확인으로 표시한다. 신규 23건 소급 정리와 이후 웹 수집의 완료 검증에 이 UI를 포함한다. 조회 UI 구현 요구이며 필드 직접 편집·승인은 별도 범위다. 상세는 DESIGN.md와 docs/remaining-work-20260915.md를 따른다.

**로컬 완료 후 AWS 이관 — 2026-09-15 사용자 확정:** 수집과 기존/신규 60필드 생성·검증, reader 및 이미지·조문 후속 처리를 로컬에서 먼저 마친 뒤, 완료 시점의 DB와 참조 파일을 일관된 snapshot으로 복사해 AWS로 이관한다. AWS에서 초기 전체 수집을 다시 수행하는 계획으로 바꾸지 않는다. 이관 기준 시점·수집 범위·잔여 제공자 미확정 상태·건수·hash·schema/코드 버전을 기록하고 AWS 복원 후 검색·본문·이미지·조문·인증을 검증한다. 로컬 원본과 백업은 유지한다. 이 지시는 계획 기록이며 지금 AWS 변경·전송을 실행하라는 승인이 아니다. 상세 순서는 docs/remaining-work-20260915.md를 따른다.

**신규·기존 60필드 동등성 — 2026-09-15 사용자 확정:** 신규 판례도 기존 corpus와 같은 60필드의 의미·표현 계약에 따라 생성·검증해야 수집·보존·정리 완료로 본다. 원문·reader·이미지/조문 후속 job 성공만으로 전체 완료를 보고하지 않는다. 기존 추출 결과와 검증된 규칙을 계승하되 확인된 추출 오류는 복제하지 않는다. 모든 값을 강제로 채우지 말고 정보 미제공·비해당·미처리·추출 실패·검증 보류를 구분하며 근거·규칙 버전·원문 참조를 남긴다. 과거 파일 경로나 처리 시각을 신규 자료에 꾸며 넣지 않는다. 신규 23건은 원문·reader/보강 처리 이력은 있으나 60필드 생성·검증 미완료다. 기존 원문·Parquet·reader를 덮어쓰지 않고 새 파생 revision과 snapshot으로 추가한다. 관리자가 웹에서 명시적으로 시작한 수집의 영속 후속 체인에 포함하며 정기 schedule/cron은 만들지 않는다. 우선순위와 완료 기준은 [현재 잔여 작업](docs/remaining-work-20260915.md)을 따른다.

**검색 판례의 완전한 보강 열람 — 2026-09-11 사용자 확정:** 검색 결과를 클릭하면 scourt에서 보존한 HTML의 문단·표·순서와 이미지/조문 위치를 바탕으로, scourt 이미지 실물과 lawgo가 해당 판례에 제공한 법령·조문 내용을 함께 보여주는 것이 최종 목표다. 기존 89,130행 corpus 전체와 앞으로 수집할 판례 모두에 필수로 적용하며 KorCounsel 프런트의 일반 검색·열람 경로에서 제공한다. 이 보강 열람은 스크레이핑·가공을 유지하는 핵심 목적이며 선택적 부가기능이 아니다. 이미지·조문 부분의 교체/보강은 원본을 덮어쓰지 않는 파생 열람 표현으로 구현하며, 기존 보강 HTML의 조문 내용도 계승한다. 저장·별도 표본 열람만으로 완료 처리하지 않고 검색→본문→이미지·조문 표시를 끝까지 검증한다. 미취득·미연결·버전 미확인은 위치별로 드러낸다. docs/legacy-enrichment-and-incremental.md의 ‘검색에서 완전한 보강 본문 열람까지’ 계약을 따른다.
...
## 현행 source 경로 조사 — 2026-09-10

- docs/source-path-validation.md의 실제 조사에 따라 기존 source ID가 현재도 항상 유효하다고 가정하지 않는다. 새 ID 후보는 old ID·legacy 이력과 함께 검토하고 자동 덮어쓰지 않는다.
- scourt 현재 본문은 src 없는 img와 contImagePath name/value로 이미지 주소를 제공할 수 있다. 원 src 부재·이름·매핑·중복 위치·해석 URL을 보존한다. img 개수만으로 취득 완료나 손실을 판단하지 않는다.
